...
def graph_builder(interval, F_real, F_predicted):
    fig = go.Figure()
    fig.add_trace(go.Scatter({'x': interval, 'y': F_real, 'name': 'real'}))
    fig.add_trace(go.Scatter({'x': interval, 'y': F_predicted, 'name': 'prediction'}))
...
if __name__ == '__main__':
    ...
    P = 4
    Eta = 0.9
    A = Prediction_NN(function, a, b)
    print(A.learn())
    er, y1, y2 = A.get_function_prediction(b, 2 * b - a, 20)
    print(er)
    print(y1)
    print(y2)
